...
export const performAdvancedLinguisticSearch = <TVerse extends VerseInput>(
  ...
  const tokenMatches = tokens.map((token) => {
      ...
      if (entry) {
        ...
        if (options.lemma && targetLemma) {
          if (lemmaIndex) {
            ...
            if (gidsFromIndex) gidsFromIndex.forEach((gid) => matchingGids.add(gid));
          } else {
    ...
    // Increase tolerance if the token is very short (3-4 chars)
    const isShortWord = token.length <= 4;
    const cutoff = hasHighQualityMatches ? 0.35 : (isShortWord ? 0.6 : 0.5);
